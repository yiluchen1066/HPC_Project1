...
/*
void square_dgemm_avx(int n, double* A, double* B, double* C)
{
  // TODO: Implement the blocking optimization
  __m256d ymm0, ymm1;
  __m256i zero = _mm256_setzero_si256();
  ...
  for (int j = 0; j < n; j = j+s)
  {
    ...
    for (int k = 0; k < n; k = k+s)
    {
      ...
      for (int i = 0; i < n; i = i+s)
      {
        ...
        {
          if (limk < n && limi < n) {
            __m256d a00, a01, a02, a03, a10, a11, a12, a13, a20, a21, a22, a23, a30, a31, a32, a33;
            __m256d c0, c1, c2, c3, b0, b1, b2, b3;
            a00 = _mm256_set1_pd(A[i+n*(k+0)]);
            a10 = _mm256_set1_pd(A[i+1+n*(k+0)]);
            a20 = _mm256_set1_pd(A[i+2+n*(k+0)]);
            a30 = _mm256_set1_pd(A[i+3+n*(k+0)]);
            b0 = _mm256_loadu_pd(&B[k+n*j]);
            c0 = _mm256_loadu_pd(&C[i+j*n]);
            c0 = _mm256_fmadd_pd(a00, b0,  );

            a01 = _mm256_set1_pd(A[i+n*(k+1)]);
            a11 = _mm256_set1_pd(A[i+1+n*(k+1)]);
            a21 = _mm256_set1_pd(A[i+2+n*(k+1)]);
            a31 = _mm256_set1_pd(A[i+3+n*(k+1)]);
            a02 = _mm256_set1_pd(A[i+n*(k+2)]);
            a12 = _mm256_set1_pd(A[i+1+n*(k+2)]);
            a22 = _mm256_set1_pd(A[i+2+n*(k+2)]);
            a32 = _mm256_set1_pd(A[i+3+n*(k+2)]);
            a03 = _mm256_set1_pd(A[i+n*(k+3)]);
            a13 = _mm256_set1_pd(A[i+1+n*(k+3)]);
            a23 = _mm256_set1_pd(A[i+2+n*(k+3)]);
            a33 = _mm256_set1_pd(A[i+3+n*(k+3)]);
          }
          for (int si = i; si < limi; si++)
          {
            double cij = C[si+sj*n]; 
